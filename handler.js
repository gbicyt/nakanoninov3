...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (opts['gconly'] && !m.chat.endsWith('g.us') && !m.fromMe)
            return conn.reply(m.chat, 'Hai Kak, Bot Sedang Dalam Mode Khusus Grub\nKakak Masuk Gc Ku Yah Biar Bisa Pake Fitur Ku >,<\n[ Link ]\nhttps://chat.whatsapp.com/J6SNWsENprgGPtMaMzUH3P',)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`Hai Sayangg Ada Yang Error Nih\n*🗂️ Plugin:* ${m.plugin}\n*👤 Sender:* ${m.sender}\n*💬 Chat:* ${m.chat}\n*💻 Command:* ${usedPrefix}${command} ${args.join(' ')}\nSemangat Fix Eror Nya Sayanggg >,<`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat Datang, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || 'Sayonaraa, @user!')).replace('@user', `${this.getName(user)}`)
